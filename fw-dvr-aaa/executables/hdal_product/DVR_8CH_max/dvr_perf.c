...
#define SUBSTREAM_WIDTH    	960
#define SUBSTREAM_HEIGHT    480
#define SUBSTREAM_LEN      ((SUBSTREAM_WIDTH * SUBSTREAM_HEIGHT * 3) / 2)
#define MAX_REC_BITSTREAM_NUM  8
#define MAX_PB_BITSTREAM_NUM   8
#define MAX_LV_BITSTREAM_NUM   8
#define ENC_FRAME_RATE         15
#define PB_FRAME_RATE          15
...
#else
	#define PATTERN_BS_FILE		"1920x1080_ref1.265"
	#define PATTERN_LEN_FILE	"1920x1080_ref1.len"
#endif
...
HD_RESULT set_rec_cap_param(VIDEO_RECORD *rec)
{
	...
	for (i = 0; i < dvr_rec_bitstream_num; i++) {
		...
		cap_out.pxlfmt = HD_VIDEO_PXLFMT_YUV420_NVX3;//cap_syscaps.pxlfmt;
		if (rec->cap_syscaps[i].max_frame_rate >= ENC_FRAME_RATE) {
			cap_out.frc = HD_VIDEO_FRC_RATIO(ENC_FRAME_RATE, rec->cap_syscaps[i].max_frame_rate);
		} else {
			cap_out.frc = HD_VIDEO_FRC_RATIO(1, 1);
		}
...
static void *playback_thread(void *arg_pb)
{
	...
	frame_period = 1000 / PB_FRAME_RATE;
...
int main(int argc, char *argv[])
{
	...
	STAMP_INFO stamps = {"400x100_argb1555.bin", {400, 100}, HD_COMMON_MEM_VB_INVALID_BLK, 0};
	...
	while (is_exit == 0) {
		...
		case 'P': case 'p':
			if (pb_div > 1) {
				...
				if (dvr_pb_bitstream_num == 1 || dvr_pb_bitstream_num == 2) { // 1ch or 2ch
					pb_div = 2;
