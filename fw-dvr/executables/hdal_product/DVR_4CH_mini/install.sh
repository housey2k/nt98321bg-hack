...
cp $SAMPLE_SRC/media_flow/liveview_1div_to_4div $PWD/image
cp $SAMPLE_SRC/media_flow/display_with_change_mode $PWD/image
cp $SAMPLE_SRC/hdal_product/module_init $PWD/image
cp $NVT_HDAL_DIR/test_cases/sys_perf/module_mem_usage $PWD/image
cp $PRODUCT_SRC/cfg_${PRODUCT}.dtb $PWD/image
cp $PRODUCT_SRC/dvr_perf $PWD/image
cp $PRODUCT_SRC/720P_30fps.h265* $PWD/image
cp $NVT_HDAL_DIR/test_cases/sys_perf/module_mem_usage $PWD/image
cp $PRODUCT_SRC/osg_88x32_rgb1555.bin $PWD/image
cp $PRODUCT_SRC/logo_720x576.jpg $PWD/image
cp $PRODUCT_SRC/clean_files.sh $PWD/image
rm $ROOTFS_DIR/rootfs/lib/*.a
