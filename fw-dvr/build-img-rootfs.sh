...
rebuild_sq() {
    ...
    # Extract exact parameters directly from unsquashfs superblock
    sq_info=$(unsquashfs -s "$orig" 2>/dev/null)

    blocksize=$(echo "$sq_info" | awk '/Block size/ {print $3}')
    comp=$(echo "$sq_info" | awk '/Compression/ {print $2}')

    # Fallbacks if extraction ever fails
    [ -z "$blocksize" ] && blocksize=262144
    [ -z "$comp" ] && comp=xz

    echo "    [Config: $comp, blocksize $blocksize]"
...
write_part() {
    ...
    # Check if the file itself is larger than the allocated partition
    if [ "$actual_size" -gt "$size" ]; then
        echo "ERROR: $file too large ($actual_size > $size)"
        exit 1
    fi

    # Check if writing this partition would overflow flash
    end=$((offset + size))
    if [ "$end" -gt "$FLASH_SIZE" ]; then
        echo "ERROR: $file would overflow flash! End offset $(printf 0x%x "$end") > FLASH_SIZE $(printf 0x%x "$FLASH_SIZE")"
        exit 1
    fi
