...
if [ "$MODEL_NAME" == "cfg_98323_4GX2_NAND_DVR_SYS" ]; then
    # [NT98323 SYS] 8CH, camera support to 960H, 720P@30, 1080P@30, 4M@15, 5M@10
	if [ "$AD_MODULE" == "techpoint" ]; then	
        modprobe tp28xx_kdrv drv_mode=0 dev_num=2 ibus=0,1 iaddr=0x88,0x8a vout_mode=3,3 video_mux=1 vout_xcap=0x03040000,0x01020000 vout_vi=0x03040000,0x01020000 vout_cdly=0x08080000 clk_dly=0x00000000,0x00000000 clk_inv=0x01010000,0x01010000 ch_map=2 clk_used=0x1 rstb_used=106 clk_driving=2  
	else	
		modprobe nvp6158_kdrv drv_mode=0 dev_num=2 ibus=0,0 iaddr=0x60,0x62 vout_mode=4,4 vout_xcap=0x0201,0x0403 vout_vi=0x0201,0x0403 vout_cdly=0x0606,0x0606 clk_dly=0x0000,0x0000 clk_inv=0x0000,0x0000 ch_map=1 clk_used=0x1 rstb_used=106 clk_driving=2
	fi	
    modprobe vcap316_common
    modprobe vcap316_host0 sync_time_div=90
else
    echo 105 > /sys/class/gpio/export
    gpio105=$(cat /sys/class/gpio/gpio105/value)
    pkgid=98323
    if [ "$gpio105" == "1" ] ; then
        echo This is 98323 System Board.
        # [NT98323 SYS] 8CH, camera support to 960H, 720P@30, 1080P@30, 4M@15, 5M@10
	    if [ "$AD_MODULE" == "techpoint" ]; then	
            modprobe tp28xx_kdrv drv_mode=0 dev_num=2 ibus=0,1 iaddr=0x88,0x8a vout_mode=3,3 video_mux=1 vout_xcap=0x03040000,0x01020000 vout_vi=0x03040000,0x01020000 clk_dly=0x00000000,0x00000000 clk_inv=0x01010000,0x01010000 ch_map=2 clk_used=0x1 rstb_used=106 clk_driving=2  
    	else	
		    modprobe nvp6158_kdrv drv_mode=0 dev_num=2 ibus=0,0 iaddr=0x60,0x62 vout_mode=4,4 vout_xcap=0x0201,0x0403 vout_vi=0x0201,0x0403 vout_cdly=0x0606,0x0606 clk_dly=0x0000,0x0000 clk_inv=0x0000,0x0000 ch_map=1 clk_used=0x1 rstb_used=106 clk_driving=2
        fi
        modprobe vcap316_common
...
modprobe kdrv_jpg
modprobe h26xenc h26x_enc_max_width=2560 h26x_enc_max_height=1440 max_total_cam_ch=45
modprobe nvt_vencrc
modprobe kflow_videoenc
modprobe h26xdec h26xd_max_width=2560 h26xd_max_height=1440 max_total_cam_ch=20
...
if [ "$AD_MODULE" == "techpoint" ]; then
    /mnt/mtd/module_init /mnt/mtd/cfg_DVR_8CH.dtb 0 1 1 1 0 &
else
	/mnt/mtd/module_init /mnt/mtd/cfg_DVR_8CH.dtb 0 2 1 1 0 &
fi
